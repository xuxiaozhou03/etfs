...
const { request, extractJsonFromCallback } = require("./utils");

exports.fetchEtfList = async () => {
  let allEtfs = [];
  let page = 1;
  let totalPages = 1;
  ...
  while (page <= totalPages) {
    ...
    const result = extractJsonFromCallback(data);
    if (!result.data) {
      break;
    }
    if (typeof result.data.diff === "object") {
      allEtfs = [
        ...allEtfs,
    ...
    } else if (Array.isArray(result.data.diff)) {
      allEtfs = [
        ...allEtfs,
